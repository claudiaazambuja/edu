...
export function FAQAccordion() {
  return (
    <div className="space-y-3">
      {faqItems.map((item) => (
        <details key={item.question} className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800/95">
          <summary className="cursor-pointer font-semibold text-ink dark:text-slate-100">{item.question}</summary>
